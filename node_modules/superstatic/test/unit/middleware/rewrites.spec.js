...
describe('static router', function() {
  ...
  beforeEach(function() {
    fs.outputFileSync('.tmp/csr.html', 'index', 'utf8');
  ...
  it('serves a route', function(done) {
    app.use(rewrites({
      rewrites: [{
        source: '/my-route', destination: '/csr.html'
      }]
    }));
  ...
  it('serves a negated route', function(done) {
    app.use(rewrites({
      rewrites: [{
        source: '!/no', destination: '/csr.html'
      }]
    }));
  ...
  it('skips if no match is found', function(done) {
    app.use(rewrites({
      rewrites: [{
        source: '/skip', destination: '/csr.html'
      }]
    }));
  ...
  describe('uses first match', function() {
    beforeEach(function() {
      fs.outputFileSync('.tmp/admin/csr.html', 'admin index', 'utf8');

      app.use(rewrites({
        rewrites: [
          {source: '/admin/**', destination: '/admin/csr.html'},
