...
module.exports = function() {
  return function(req, res, next) {
    ...
    return res.superstatic.provider(req, pathname).then(function(result) {
      if (result) {
        // If we are using cleanURLs, we'll trim off any `.html` (or `/csr.html`), if it exists.
        if (cleanUrlRules) {
      ...
      // We want to check for some other files, namely an `csr.html` if this were a directory.
        ...
        if (cleanUrlRules) {
          ...
          return res.superstatic.provider(req, appendedPath).then(function(appendedPathResult) {
            if (appendedPathResult) {
              ...
              // If we've gotten this far and still have `/csr.html` on the end, we want to remove it from the URL.
              if (_.endsWith(appendedPath, '/csr.html')) {
                return res.superstatic.handle({redirect: normalizeRedirectPath(pathutils.removeTrailingString(appendedPath, '/csr.html') + search)});
              }
